...
FROM employee e
WHERE e.e_salary =
      (SELECT max(ei.e_salary) FROM employee ei WHERE e.dept_no = ei.dept_no);
...
select dept_no,count(e_no)as "部门人数" from employee group by dept_no order by dept_no;
-- 在employee表中，计算不同类型职工的总工资数。
select e_job,sum(e_salary) as "总工资" from employee group by e_job;
-- 在employee表中，计算不同部门的平均工资。
select dept_no,avg(e_no)as "部门平均工资" from employee group by dept_no order by dept_no;
...
select * from fruits;
select f_name,s_id from fruits order by s_id;
...
select count(c_email)as 有电子邮箱顾客数 from customers where c_email is not null;

-- 在orderitems表中，使用COUNT()函数统计不同订单号中订购的水果种类
select o_num,count(f_id)as 水果种类 from orderitems group by o_num;
-- 在orderitems表中查询30005号订单一共购买的水果总量
select o_num,sum(quantity)as 总量 from orderitems where o_num = 30005;
